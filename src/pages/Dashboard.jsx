...
/**
 * Sample single-file React prototype for IKD Dashboard.
 * TailwindCSS required.
 *
 * How to run:
 * 1. create-vite app or CRA, install Tailwind per docs
 * 2. replace App.jsx with this content
 * 3. start dev server
 */
  ...
  return (
    <header className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white">
      ...
      <div className="flex items-center gap-4">
        <div className="text-slate-500 text-sm">Admin Dukcapil</div>
...
function RegionCard({ item }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm flex-1 min-w-[220px]">
      ...
      <div className="mt-4 flex items-center gap-4">
        <div className="w-16 h-16 rounded-full flex items-center justify-center bg-slate-50 text-lg font-semibold">{Math.round(item.persen)}%</div>
        <div className="flex-1">
          <div className="text-sm text-slate-600">Realisasi: <span className="font-medium text-slate-800">{item.realisasi.toLocaleString()}</span></div>
          <div className="text-xs text-slate-400 mt-1">Dari {item.jumlah_rekam.toLocaleString()} wajib KTP</div>
          <div className="mt-3">
            <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
              <div className={`h-2 rounded-full ${status.color === "green" ? "bg-green-500" : status.color === "yellow" ? "bg-yellow-400" : "bg-red-400"}`} style={{ width: `${Math.min(item.persen, 100)}%` }} />
            </div>
          ...
          {wilayah.map((w) => {
            ...
            return (
              <tr key={w.id} className="border-t">
                ...
                <td>{w.persen}%</td>
                <td>
                  <span className={`px-3 py-1 rounded-full text-xs ${st.color === "green" ? "bg-green-100 text-green-700" : st.color === "yellow" ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`}>
                    {st.label}
                  </span>
                </td>
              </tr>
